...
def extract_new_trips(conn, watermark):
    extract_newtrip_sql = """
      SELECT
        t.trip_id,
        t.driver_id,
    ...
    ORDER BY t.requested_at
        """
    return extract(conn,extract_newtrip_sql)
...
def transform(oltp_row, lookups):
    ...
    for row in oltp_row:
        ...
        time_key = int(row["requested_at"].strftime("%H%m%s"))
...
def load_fact_trips(conn, fact_data):
 ...
 INSERT INTO fact_trips
    (source_trip_id, date_key, time_key, driver_key, vehicle_key, passenger_key,
...
def get_watermark(conn):
    with conn.cursor() as curr:
        logger.info(f"Getting the Watermark")
        curr.execute("""
...
def main():
    ...
    try:
        ...
        watermark = get_watermark(dst_conn)       #create a function to Read Watermark

        if watermark is None:
            logger.info("Null waterwark. Load everything.")
            rows = extract_all_trips(src_conn)        #When watermark = None,i.e Null watermark. ETL should interpret None as Initial load and load ALL trips.
        else:
            logger.info("New waterwark found. Only load newer trips.")
            rows = extract_new_trips(src_conn, watermark) #Pass Watermark into Extract
